...
本项目基于领域驱动设计（DDD）理念构建了现代化的客户端架构，并严格隔离了开源基础底座与商业级特性实现。

```text
WeMediaBaby/
├── .venv/                  # Python 虚拟环境
├── config/                 # 全局配置模板与环境变量（敏感配置已脱敏）
├── docs/                   # 核心架构与业务设计文档（包含内部私有说明，整体不在开源主干）
├── resources/              # 静态资源 (Icons、StyleSheets、UI 图片等)
├── scripts/                # 自动化构建与辅助脚本：打包(build)、自动化发版(release)、测试(test)
...
│   ├── plugins/            # 平台驱动引擎插件
│   │   ├── community/      # [开源模块] 抖音、快手等开放平台发布驱动
│   │   └── pro/            # [商业闭源] 视频号、小红书、B站等另外 8 个高优平台驱动
│   ├── pro_features/       # [商业闭源] 批量并发创建、跨平台数据大屏、互动管理中心等高阶业务组件
│   ├── proprietary/        # [商业闭源] 云端 API 鉴权、用量控制、订阅会员体系
...
**🛡️ 目录隔离安全原则**：
带 `[商业闭源]` 标记的核心代码夹已被 `.gitignore` 强力隔离，**绝对不会出现在公开的开源代码库中**。开源版的基础底座在启动时会自动探测这些目录，一旦发现缺失，会自动平滑降级（例如自动隐藏 Pro 版特权的界面入口），确保开源版依然是一个纯净且可独立编译运行的发布工具！
