...
> 一款面向自媒体创作者的桌面端**多平台作品发布工具**。你把作品配置成“发布任务”，写入发布列表，软件通过浏览器自动化在平台页面完成上传与提交。

## 你能用它做什么（开源版）

- **发布任务**：创建单条发布任务（视频/图文等按页面提供的入口）
- **批量执行**：发布列表支持一次性执行多条任务（批量跑队列）
- **账号与浏览器**：账号管理、浏览器环境与反检测相关能力（以开源版现状为准）
- **二次开发**：提供可运行的 UI/发布列表/执行链路框架，便于扩展
