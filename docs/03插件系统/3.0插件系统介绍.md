...
graph TB
    ...
    subgraph pro["高密商业平台层 (pro/)"]
        ...
        XHS[xiaohongshu/<br/>小红书专项代码<br/>未注册发布入口]
    end
...
| 平台 | platform_id | 分类 | 目录 | `PluginManager` 注册状态 | 当前说明 |
| -------------- | -------------- | ---- | ---- | ----------------------- | -------- |
| **抖音** | `douyin` | Community | `src/plugins/community/douyin/` | 登录/发布已注册 | OSS 默认开放的基准平台 |
| **快手** | `kuaishou` | Community | `src/plugins/community/kuaishou/` | 登录/发布已注册 | 代码注册，按发行模式与权限入口开放 |
| **视频号** | `wechat_video` | Pro | `src/plugins/pro/wechat_video/` | 登录/发布已注册 | Pro feature flag 开放 |
| **B站** | `bilibili` | Pro | `src/plugins/pro/bilibili/` | 登录/发布已注册 | Pro 注册表平台 |
| **微博** | `weibo` | Pro | `src/plugins/pro/weibo/` | 登录/发布已注册 | Pro 注册表平台 |
| **头条** | `toutiao` | Pro | `src/plugins/pro/toutiao/` | 登录/发布已注册 | Pro 注册表平台 |
| **百家号** | `baijiahao` | Pro | `src/plugins/pro/baijiahao/` | 登录/发布已注册 | Pro 注册表平台 |
| **多多视频** | `duoduoshipin` | Pro | `src/plugins/pro/duoduoshipin/` | 登录/发布已注册 | Pro 注册表平台 |
| **企鹅号** | `qiehao` | Pro | `src/plugins/pro/qiehao/` | 登录/发布已注册 | Pro 注册表平台 |
| **小红书** | `xiaohongshu` | Pro 专项 | `src/plugins/pro/xiaohongshu/` | 未注册 | 账号普通 Chrome 登录/同步和诊断代码存在；统一发布入口目前会返回“未找到平台插件” |

> 抖音插件是当前实现最完整的范本。其他已注册平台均采用步骤链模式，但**具体步骤数量和内容由各平台自行定义**。小红书相关文档用于记录专项代码与历史诊断材料，不能视为主线发布入口已开放。
...
- **加载方式**：默认按需加载；设置 `PLUGIN_EAGER_INIT=1` 时才在启动期全量导入，主要用于兼容旧行为或排查打包收集问题
- **初始化**：应用启动时调用 `PluginManager.initialize()` 注册平台映射，插件实例在获取时懒加载
- **获取插件**：
...
**新增平台时**：需要同步 `PLUGIN_REGISTRY`、按需导入路径、feature flag、平台配置、设置页启用列表与必要测试；仅新增目录不会让发布入口可用。
...
- **各平台**：视频号步骤 8（定时）、10（原创）、11（发表）已按上述方式收敛；抖音、快手等已注册平台按步骤链逐步收敛滚轮物理化与鼠标真实化。小红书相关代码仍处于未注册专项状态，接入主线前需要重新核对自动发布动作。
...
| 04    | [小红书插件功能说明](04小红书插件/小红书插件功能说明.md) / [技术文档](04小红书插件/小红书插件技术文档.md) | 专项代码、账号同步、历史 DOM 分析与平台扩展诊断；发布入口未注册 |