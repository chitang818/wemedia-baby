...
- 浏览器层防风控由基础设施 (`src/infrastructure/`) 提供，插件拿到的 `page` 已在防风控环境中
- 发布层防风控由公共模块 (`src/infrastructure/anti_risk/`) 提供，插件在步骤中按需调用
...
- **各平台**：视频号步骤 8（定时）、10（原创）、11（发表）已按上述方式收敛；快手等社区插件在发现同类问题时同样改为真实鼠标，**不再**用 `evaluate` 内 `el.click()` 作为定时确认等关键步骤的兜底。
