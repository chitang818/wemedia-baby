...
**文档简介**：本文档面向开发者和 AI 工具，记录小红书专项代码、历史发布步骤链、closed Shadow 发布钮、定时设置与提交逻辑、选择器探针，以及发布失败时的平台扩展诊断包。注意：截至 v1.4.3，小红书登录/发布插件虽可正常调用，但因其极易触发平台反自动化检测，属于高风控功能。实际开放时请务必配合完善的浏览器指纹与沙盒环境，并谨慎控制使用频率。改版时请同步更新页首「文档版本」和「最后更新时间」。
...
位于 `steps/step_runner.py`，继承 `src/plugins/core/step_runner.py::GenericStepRunner`。
