...
class VideoLibraryPage(TrackedTaskMixin, BasePage):
    ...
    def _setup_content(self):
        ...
        self._table = MediaLibraryTableView(
            table_card,
            kind=MediaLibraryTableModel.KIND_VIDEO,
        )
    ...
    def _populate_table(self, items: List[_VideoItem]) -> None:
        """Render scanned videos through the Model/View table."""
        if not self._table:
            return
    ...
    def _schedule_refresh_usage_marks(self, gen: int) -> None:
        ...
        try:
            self._create_tracked_task(_run(), name="video_library.refresh_usage_marks")
        except Exception:
            return
    ...
    def _update_metadata_columns(self) -> None:
        """Refresh video metadata columns in place without rebuilding rows."""
        if not self._table:
            return
        self._table.notify_columns_changed([_COL_DURATION, _COL_RESOLUTION, _COL_ORIENT])
