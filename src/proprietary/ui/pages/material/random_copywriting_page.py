...
class RandomCopywritingPage(TrackedTaskMixin, BasePage):
    ...
    def __init__(self, parent: Optional[QWidget] = None):
        ...
        self.setObjectName("RandomCopywritingPage")
    ...
    def _open_sync_file_folder(self):
        ...
        path = QSettings("WeMediaBaby", "媒小宝").value("app/random_copywriting_last_import_path")
    ...
    def _update_sync_path_label(self):
        source = QSettings("WeMediaBaby", "媒小宝").value("app/random_copywriting_active_source", "none")
    ...
    def _on_unbind_clicked(self):
        ...
        if show_confirm(self, "确认解除绑定", "解除绑定不会删除本地已有的文案，但之后您需要重新绑定数据源才能同步更新。\\n确认解除吗？"):
            QSettings("WeMediaBaby", "媒小宝").setValue("app/random_copywriting_active_source", "none")
            self._update_sync_path_label()
    ...
    async def _reload_all(self) -> None:
        ...
        try:
            # 1. 尝试获取所有分类
            categories = await RandomCopywritingRepository.get_all_categories()
            if not categories:
                categories = ["全部"]
    ...
    async def _load_current_page_data(self):
        ...
        try:
            # 取消分页，一次性加载当前分类的所有记录
            rows = await RandomCopywritingRepository.list_items(
    ...
    def _on_bind_excel_clicked(self):
        ...
        dlg = AppMessageBoxBase(self, header_title="导入随机文案")
        desc = BodyLabel(
            "您可以从 Excel 模板批量导入随机文案，每次导入将全量替换本地数据。\\n\\n"
        ...
        btn_download = PushButton("下载随机文案库模板", dlg, FluentIcon.DOWNLOAD)
        ...
        def _download_template():
            ...
            default_path = os.path.join(desktop_dir, "随机文案库模板.xlsx") if os.path.exists(desktop_dir) else "随机文案库模板.xlsx"

            save_path, _ = QFileDialog.getSaveFileName(self, "保存随机文案库模板", default_path, "Excel 文件 (*.xlsx)")
            if save_path:
        ...
        QSettings("WeMediaBaby", "媒小宝").setValue("app/random_copywriting_last_import_path", file_path)
        QSettings("WeMediaBaby", "媒小宝").setValue("app/random_copywriting_active_source", "excel")
        self._update_sync_path_label()
    ...
    async def _import_excel(self, file_path: str):
        try:
            from src.infrastructure.common.excel_random_copywriting_importer import parse_excel
        ...
        if not items:
            msg = "导入失败：未在文件中找到任何符合规范（仅要求包含“文案”或“内容”列）的随机文案数据，请检查 Excel 表头是否正确。"
            if base_errors:
                msg += "\\n" + "\\n".join(base_errors[:5])
            InfoBar.warning(
                title="导入失败",
        ...
        try:
            stats = await RandomCopywritingRepository.bulk_import(items, overwrite_by_work_id=True, clear_first=True)
        except Exception as e:
    ...
    async def _do_feishu_sync_with_result(self, result: dict):
        ...
        try:
            from src.proprietary.services.feishu.feishu_random_copywriting_sync import FeishuRandomCopywritingSyncService
            ...
            sync_service = FeishuRandomCopywritingSyncService()
