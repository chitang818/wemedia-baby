...
class BatchTaskCreationPage(BasePage):
    """批量视频任务创建页面"""
    ...
    def closeEvent(self, event):
        for task in list(self._pending_tasks):  # type: ignore
            if not task.done():
                task.cancel()
    ...
    def _setup_content(self):
        ...
        # 须先于批量发布设置卡片构建：设置同步依赖「声明原创」复选框（在账号操作栏描述卡片内）
    ...
    def _create_account_import_card(self) -> QWidget:
        ...
        self._batch_declare_original_check = CheckBox("声明原创", card_extra)
        self._batch_declare_original_check.stateChanged.connect(self._on_batch_declare_original_toggled)
        extra_l.addWidget(self._batch_declare_original_check, 0, Qt.AlignmentFlag.AlignVCenter)
    ...
    def _schedule_auto_match_if_enabled(self, *, force_run: bool = False) -> None:
        ...
        QTimer.singleShot(10, _start_match_task)
    ...
    def _on_add_video_clicked(self):
        ...
        if accepted:
            ...
            if dlg.choice == "files":
                QTimer.singleShot(0, self._on_import_files)
            elif dlg.choice == "folder":
                QTimer.singleShot(0, self._on_import_folder)
            elif dlg.choice == "library":
                QTimer.singleShot(0, self._on_choose_from_library)
    ...
    def _on_publish_time_clicked(self):
        ...
        if dialog.exec() == int(QDialog.DialogCode.Accepted):
            self.time_slots = dialog.get_time_slots()
    ...
    def _load_publish_list_exclude_paths_sync(self) -> set:
        ...
        try:
            task = asyncio.ensure_future(self._load_publish_list_exclude_paths())
            ev = QEventLoop(self)
            task.add_done_callback(lambda _: ev.quit())
            ev.exec()
    ...
    def _refresh_task_status_reminder(self) -> None:
        ...
        QTimer.singleShot(10, _schedule_status_update)
    ...
    def _sync_batch_publish_settings_ui(self, *, video_auto_match: Optional[bool] = None) -> None:
        ...
        self._batch_declare_original_check.blockSignals(True)
        ...
        try:
            ...
            # 5. 原创与其他
            self._batch_declare_original_check.setChecked(
                bool(getattr(self, "declare_original_checked", True))
            )
            ...
            self._batch_declare_original_check.blockSignals(False)
    ...
    def _on_batch_desc_mode_changed(self, _index: int) -> None:
        ...
        if mode == CopywritingMatchMode.NONE:
            QTimer.singleShot(100, self._open_publish_description_dialog)
    ...
    def _on_batch_declare_original_toggled(self, _state: int = 0) -> None:
        self.declare_original_checked = self._batch_declare_original_check.isChecked()
        save_persisted_declare_original(self.declare_original_checked)
        self._refresh_preview()
    ...
    def _create_preview_card(self) -> QWidget:
        ...
        self.preview_table.setHorizontalHeaderLabels([
            "平台", "平台昵称", "文件", "封面", "作品标题", "作品简介", "发布时间", "声明原创", "购物车", "团购", "位置"
        ])
        ...
        self.preview_table.setColumnWidth(7, 72)   # 声明原创
    ...
    def _preview_show_declare_original_checked(
        self, *, task_platform: str, privacy_marks_original: bool
    ) -> bool:
        """预览「声明原创」列：仅当发布目标含视频号且 privacy 中标记为原创时显示 ✅。"""
        if not privacy_marks_original:
            return False
        p = (task_platform or "").strip()
        if p == "account_group":
            return self._selected_targets_include_wechat_video()
        return p == "wechat_video"

    def _account_placeholder_declare_original_display(self, acc: dict, is_orig_checked: bool) -> str:
        """仅有账号占位行时的「声明原创」列文案。"""
        if not is_orig_checked:
            return TASK_FIELD_EMPTY_DISPLAY
        if acc.get("_type") == "group":
            ok = self._group_placeholder_has_wechat_video(acc)
        else:
            ok = self._account_entry_is_wechat_video(acc)
        return "✅" if ok else TASK_FIELD_EMPTY_DISPLAY
    ...
    def _refresh_preview(self, *, skip_material_stats_reminder: bool = False):
        ...
        is_orig_checked = getattr(self, "declare_original_checked", True)
        n_vid = br.n_vid
        n_time = br.n_time
        ...
        if br.branch == "full":
            ...
            for row, t in enumerate(br.tasks):
                try:
                    ps = json.loads(t.get("privacy_settings") or "{}")
                    is_orig = bool(ps.get("is_original", False))
                except Exception:
                    is_orig = False
                ...
                actual_is_orig = self._preview_show_declare_original_checked(
                    task_platform=str(plat), privacy_marks_original=is_orig,
                )
                ...
                self._fill_preview_row(row, [
                    ...
                    "✅" if actual_is_orig else TASK_FIELD_EMPTY_DISPLAY,
            ...
            for row, t in enumerate(br.no_video_placeholder_rows):
                ...
                self._fill_preview_row(row, [
                    ...
                    self._preview_time_column_text(t),
                    self._account_placeholder_declare_original_display(
                        {"platform": plat}, is_orig_checked
                    ),
                    format_cart_info_table_cell(t.get("cart_info")),
                    "✅" if (t.get("anchor_info") or "").strip() else TASK_FIELD_EMPTY_DISPLAY,
                    format_poi_table_cell_display(
            ...
            for row, t in enumerate(br.no_time_placeholder_rows):
                ...
                self._fill_preview_row(row, [
                    ...
                    self._preview_time_column_text(t),
                    self._account_placeholder_declare_original_display(t, is_orig_checked),
                    format_cart_info_table_cell(t.get("cart_info")),
                    "✅" if (t.get("anchor_info") or "").strip() else TASK_FIELD_EMPTY_DISPLAY,
                    format_poi_table_cell_display(
                        t.get("poi_info"),
