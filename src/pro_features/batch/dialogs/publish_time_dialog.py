...
class PublishTimeDialog(AppMessageBoxBase):
    ...
    def __init__(
        self,
        initial_slots: Optional[List[str]] = None,
        owner_count: int = 0,
        parent=None,
    ):
        super().__init__(parent, header_title="设置发布时间排期")  # type: ignore
        # 批量场景仅支持定时发布，不再提供「立即发布」选项
        self.time_slots = list(initial_slots) if initial_slots else []
        ...
        self._time_slots_render_snapshot: Optional[List[str]] = None
    ...
    def _init_single_mode_page(self):
        ...
        card_lay.addWidget(btn_add_time, 0, Qt.AlignmentFlag.AlignLeft)
    ...
    def _create_schedule_result_empty_placeholder(self, parent: QWidget) -> QWidget:
        ...
        col.addWidget(title)
        hint = CaptionLabel(
            "在左侧选择日期与时间并添加排期后，将在此显示完整列表", wrap)
        hint.setAlignment(Qt.AlignmentFlag.AlignCenter)
        hint.setWordWrap(True)
        hint.setStyleSheet("color: #999999; font-size: 12px;")
        col.addWidget(hint)
    ...
    def _update_statistics_panel(self):
        ...
        if chip_range is not None:
            if total == 0:
                ...
                _ds = sorted({ts.split(" ")[0] for ts in self.time_slots})
                if len(_ds) == 1:
        ...
        if total == 0:
            ...
            return
        dates_sorted = sorted({ts.split(" ")[0] for ts in self.time_slots})
        ...
        per_day = Counter(ts.split(" ")[0] for ts in self.time_slots)
        ...
        d.setToolTip("排期覆盖的自然日数（按右侧结果列表统计）")
