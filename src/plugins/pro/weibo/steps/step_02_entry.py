...
"""
步骤2：进入发布页
文件路径: src/plugins/pro/weibo/steps/step_02_entry.py

流程：
  1. 优先尝试直接导航到视频发布页 URL（/upload/channel）
  2. 若直接导航失败或未检测到发布页特征，回退到首页查找发布入口点击
...
PUBLISH_URL = "https://weibo.com/upload/channel"
...
class EnterPublishEntryStep(BasePublishStep):
    ...
    async def _check_publish_page_loaded(self, page: Page) -> bool:
        ...
        try:
            if "upload" in page.url and "weibo.com" in page.url:
                return True
        except Exception:
            pass
