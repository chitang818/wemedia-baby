...
"""
新浪微博发布插件（步骤链版本）
文件路径：src/plugins/pro/weibo/publish_plugin.py

采用与抖音/视频号/B站插件一致的「步骤链 + StepRunner」架构：
    步骤1: 导航首页
    步骤2: 进入发布页
    步骤3: 上传素材（视频）
    步骤4: 填写描述（标题、简介、话题标签）
...
def _platform_config() -> Dict[str, Any]:
    """从内置 config/platforms/weibo.json 读取平台配置。"""
    return load_platform_bundle("weibo")


def _load_limits() -> Dict[str, int]:
    """优先从 config/platforms/weibo.json 读取 limits，否则从插件 config.json，否则使用默认值。"""
    try:
...
class WeiboPublishPlugin(PublishPluginInterface):
    """新浪微博发布插件 — 步骤链模式（7步）"""

    UPLOAD_URL = "https://weibo.com/upload/channel"
    LOGIN_URL_KEYWORDS = ["passport.weibo", "sso/signin"]
    LOGIN_TEXT_INDICATORS = ["扫码登录", "手机号登录", "账号登录"]
    ...
    def get_form_schema(self, content_type: str = "video") -> List[FormField]:
        """返回发布表单定义。"""
        limits = _load_limits()
        title_max = limits.get("title_max_length", 40)
        ...
        schema = [
            FormField(
                ...
                label="标题",
                field_type="text",
                placeholder=f"输入视频标题（最多{title_max}字）",
    ...
    async def publish(
        ...
        try:
            ...
            logger.error(f"新浪微博发布插件异常: {e}", exc_info=True)
