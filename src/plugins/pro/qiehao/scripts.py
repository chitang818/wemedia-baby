"""
企鹅号插件 JavaScript 脚本
文件路径: src/plugins/pro/qiehao/scripts.py
"""
...
(function() {
    try {
        ...
        var isOMPage = currentUrl.indexOf('om.qq.com') !== -1;
        var isLoginPage = currentUrl.indexOf('/login') !== -1
                       || currentUrl.indexOf('登录') !== -1;

        // 方法1：检测关键 Cookie
        try {
            var cookies = document.cookie.split(';');
            var keyCookieNames = ['omtoken', 'omuid', 'uin', 'skey', 'luin', 'lskey'];
            var foundCookies = [];
        ...
        var elementIndicators = [
            ...
            { selector: 'div[class*="account-name"]', description: '账号名' }
        ];
        ...
        // 综合判断
        var hasOmToken = result.cookies.indexOf('omtoken') !== -1;
        var hasOmUid = result.cookies.indexOf('omuid') !== -1;
        var hasUin = result.cookies.indexOf('uin') !== -1;
        var hasUserElements = foundElements.length > 0;

        if (username && username.length > 0) {
            result.loggedIn = true;
            result.method = 'username_found';
        } else if (hasOmToken && isOMPage && !isLoginPage) {
            result.loggedIn = true;
            result.method = 'cookie_and_page';
        } else if (hasUin && hasUserElements) {
            result.loggedIn = true;
            result.method = 'cookie_and_elements';
        }

        result.debug.push('hasOmToken: ' + hasOmToken);
        result.debug.push('hasOmUid: ' + hasOmUid);
        result.debug.push('hasUin: ' + hasUin);
        result.debug.push('isOMPage: ' + isOMPage);
