...
"""
企鹅号发布插件（步骤链版本）
文件路径：src/plugins/pro/qiehao/publish_plugin.py
...
def _platform_config() -> Dict[str, Any]:
    """从内置 config/platforms/qiehao.json 读取平台配置。"""
    return load_platform_bundle("qiehao")


def _load_limits() -> Dict[str, int]:
    """优先从 config/platforms/qiehao.json 读取 limits，否则从插件 config.json，否则使用默认值。"""
    try:
...
class QiehaoPublishPlugin(PublishPluginInterface):
    """企鹅号发布插件 — 步骤链模式（7步）"""

    UPLOAD_URL = "https://om.qq.com/video/videoPublish"
    LOGIN_URL_KEYWORDS = ["om.qq.com"]
    LOGIN_TEXT_INDICATORS = ["QQ登录", "微信登录", "登录"]
    ...
    async def publish(
        ...
        try:
            ...
            logger.error(f"企鹅号发布插件异常: {e}", exc_info=True)
