...
"""
步骤2：进入发布页
文件路径: src/plugins/pro/qiehao/steps/step_02_entry.py

流程：
  1. 优先尝试直接导航到发布页 URL（/video/videoPublish）
...
PUBLISH_URL = "https://om.qq.com/video/videoPublish"
...
class EnterPublishEntryStep(BasePublishStep):
    ...
    async def _check_publish_page_loaded(self, page: Page) -> bool:
        ...
        try:
            url = page.url
            if ("videoPublish" in url or "publish" in url) and "om.qq.com" in url:
                return True
        except Exception:
            pass
