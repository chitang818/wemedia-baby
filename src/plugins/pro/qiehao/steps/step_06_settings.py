...
"""
步骤6：发布设置
文件路径: src/plugins/pro/qiehao/steps/step_06_settings.py

流程（依次执行）：
  1. 页面滚动到底部暴露设置区
  2. 分类选择（企鹅号内容需选择分类，如无指定则保持默认推荐分类）
  3. 原创声明（可选）
  4. 发布时间：
...
字段依赖：
  ...
  - metadata['privacy_settings']: 隐私设置（含 is_original 原创声明）
...
class PublishSettingsStep(BasePublishStep):
    ...
    async def execute(self, page: Page, file_path: str, metadata: Dict[str, Any]) -> StepOutcome:
        ...
        # ── 1. 原创声明（可选） ──
        privacy_settings = metadata.get("privacy_settings") or {}
