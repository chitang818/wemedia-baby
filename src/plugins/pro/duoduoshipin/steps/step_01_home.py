...
"""
步骤1：导航首页
文件路径: src/plugins/pro/duoduoshipin/steps/step_01_home.py

流程：
  1. 导航至多多视频创作者中心首页：https://live.pinduoduo.com/creator/index
  2. 等待 DOM 加载完成及短暂延迟
  3. 执行风控拦截检查（RISK_MODAL）
  4. 执行登录态探测（LOGIN_EXPIRED_INDICATORS）
  5. 验证是否处于创作者中心域内（live.pinduoduo.com/creator）
...
class NavigateHomeStep(BasePublishStep):
    """导航到多多视频创作者中心首页，并做基础登录/风控检查。"""

    CREATOR_HOME_URL = "https://live.pinduoduo.com/creator/index"
    ...
    async def execute(self, page: Page, file_path: str, metadata: Dict[str, Any]) -> StepOutcome:
        ...
        logger.info(f"导航至多多视频创作者中心: {self.home_url}")
        ...
        try:
            ...
            try:
                ...
                login_keywords = ["登录已过期", "请先登录", "请重新登录", "登录失效"]
            ...
            # 4. 确认在创作者中心域内
            if "live.pinduoduo.com" in current_url:
                logger.info("已确认处于多多视频创作者中心域内")
                return None

            return PublishResult(
                success=False,
                error_message=f"无法确认已进入创作者中心，当前 URL: {current_url}",
            )
