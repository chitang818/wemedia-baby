...
"""
步骤2：进入发布页
文件路径: src/plugins/pro/duoduoshipin/steps/step_02_entry.py

流程：
  1. 优先尝试直接导航到发布页 URL（/creator/short-video）
  2. 若直接导航失败或未检测到发布页特征，回退到首页查找「发布视频」按钮点击
...
PUBLISH_URL = "https://live.pinduoduo.com/creator/short-video"
...
class EnterPublishEntryStep(BasePublishStep):
    ...
    async def execute(self, page: Page, file_path: str, metadata: Dict[str, Any]) -> StepOutcome:
        ...
        # 策略2：从首页查找「发布视频」按钮点击
        logger.info("直接导航失败，尝试从首页查找发布入口…")
    ...
    async def _check_publish_page_loaded(self, page: Page) -> bool:
        ...
        try:
            if "short-video" in page.url and "live.pinduoduo.com" in page.url:
                return True
        except Exception:
            pass
