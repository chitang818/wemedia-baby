...
"""
步骤7：点击发布
文件路径: src/plugins/pro/toutiao/steps/step_07_submit.py
...
class SubmitStep(BasePublishStep):
            ...
            for _ in range(10):
                ...
                try:
                    ...
                    for kw in Selectors.VERIFY["SUCCESS_URL_KEYWORDS"]:
                        if kw in url and "upload" not in url.split("/")[-1]:
                            detected = True
    ...
    async def _verify_publish_result(self, page: Page, metadata: Dict[str, Any]) -> PublishResult:
        ...
        try:
            ...
            for kw in Selectors.VERIFY["SUCCESS_URL_KEYWORDS"]:
                if kw in current_url and "/upload" not in current_url.split("/")[-1]:
        ...
        try:
            ...
            current_url = page.url
            if "/upload" not in current_url and "mp.toutiao.com" in current_url:
