...
"""
步骤6：发布设置
文件路径: src/plugins/pro/toutiao/steps/step_06_settings.py

流程（依次执行）：
  1. 页面滚动到底部暴露设置区
  2. 原创声明：根据 original 字段勾选「声明原创」
  3. 发布时间：
     - 若 schedule_time 为空，则默认立即发布
     - 若有定时时间，设置定时发布

字段依赖：
  - metadata['original']: 是否声明原创 (true/false)
...
class PublishSettingsStep(BasePublishStep):
    ...
    async def execute(self, page: Page, file_path: str, metadata: Dict[str, Any]) -> StepOutcome:
        ...
        # ── 1. 原创声明 ──
        original = metadata.get("original", False)
        if isinstance(original, str):
            original = original.lower() in ("true", "1", "yes")
