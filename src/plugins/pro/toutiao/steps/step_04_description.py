...
"""
步骤4：填写描述（标题、简介、标签）
文件路径: src/plugins/pro/toutiao/steps/step_04_description.py

流程：
  1. 标题填写：定位标题输入框（TITLE_INPUT），清空后逐字输入
  2. 简介/描述填写：
     - 定位描述编辑器（DESC_EDITOR）
     - 拟人点击聚焦，全选清空旧内容
     - 逐字输入 description 全文
...
字段依赖：
  - metadata['title']: 标题（头条号限 30 字）
  - metadata['description']: 简介/描述
  - metadata['tags']: 标签列表
                    ...
                    try:
                        ...
                        if await tag_input.count() > 0 and await tag_input.is_visible():
                            ...
                            try:
                                ...
                                await page.wait_for_timeout(wait_ms(500))
                            logger.info(f"已输入标签: {tag_text}")
                            break
                    except Exception:
                        continue
            USER_LOG.info(f"[步骤4 作品描述] ✓ 标签已填写，数量={len(tags)}")
