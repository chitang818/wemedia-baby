...
"""
步骤1：导航首页
文件路径: src/plugins/pro/bilibili/steps/step_01_home.py

流程：
  1. 导航至B站创作中心首页：https://member.bilibili.com/platform/home
  2. 等待 DOM 加载完成及短暂延迟
  3. 执行风控拦截检查（RISK_MODAL）
  4. 执行登录态探测（LOGIN_EXPIRED_INDICATORS）
  5. 验证是否处于创作中心域内（member.bilibili.com）
...
class NavigateHomeStep(BasePublishStep):
    ...
    async def execute(self, page: Page, file_path: str, metadata: Dict[str, Any]) -> StepOutcome:
        ...
        logger.info(f"导航至B站创作中心: {self.home_url}")
        ...
        try:
            ...
            try:
                ...
                login_keywords = ["登录已过期", "请先登录", "请重新登录"]
            ...
            if "member.bilibili.com" in current_url:
                logger.info("已确认处于B站创作中心域内")
                return None

            return PublishResult(
                success=False,
                error_message=f"无法确认已进入B站创作中心，当前 URL: {current_url}",
            )
