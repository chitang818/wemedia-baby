...
"""
哔哩哔哩发布插件（步骤链版本）
文件路径：src/plugins/pro/bilibili/publish_plugin.py
...
def _platform_config() -> Dict[str, Any]:
    """从内置 config/platforms/bilibili.json 读取平台配置。"""
    return load_platform_bundle("bilibili")


def _load_limits() -> Dict[str, int]:
    """优先从 config/platforms/bilibili.json 读取 limits，否则从插件 config.json，否则使用默认值。"""
    try:
...
class BilibiliPublishPlugin(PublishPluginInterface):
    """哔哩哔哩发布插件 — 步骤链模式（7步）"""
    ...
    def get_form_schema(self, content_type: str = "video") -> List[FormField]:
        """返回发布表单定义。"""
        limits = _load_limits()
        title_max = limits.get("title_max_length", 80)
    ...
    async def publish(
        ...
        try:
            ...
            logger.error(f"哔哩哔哩发布插件异常: {e}", exc_info=True)
