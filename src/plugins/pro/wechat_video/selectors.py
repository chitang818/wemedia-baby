...
class Selectors:
    ...
    PUBLISH = {
        ...
        # ===== 步骤11 附属：点击「发表」后可能出现的「广告分成 / 创作分成计划」引导弹窗 =====
        # 文案含「声明原创的视频有机会获得广告分成」等；未勾选任务「声明原创」时点「直接发表」。
        # 实现见 step_11_submit._handle_post_submit_original_revenue_modal（遍历主文档 + wujie shadow 内 .weui-desktop-dialog）

        # 步骤11：发表按钮（Shadow 内 querySelector 仅能用标准 CSS，勿写 Playwright 专有 :has-text）
        # DOM 报告 L1 由 step_11 内 page.get_by_role('button', name='发表') 优先尝试
        "SUBMIT_BTN": [
