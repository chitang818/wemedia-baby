...
"""
步骤 8：定时发表设置
文件路径：src/plugins/pro/wechat_video/steps/step_08_schedule.py
...
操作序列（来自《视频号定时发布 - 时间选择器 DOM 分析报告》第七节验证结果）：
  ...
  定时发布：
    ...
    6. 真实点击目标小时 li
    7. 真实点击目标分钟 li
    8. 关闭弹窗：picker 矩形外侧点击 → 点「声明原创」标签 → Escape 兜底
...
async def _read_schedule_picker_state(page: Page) -> Dict[str, Any]:
    """读取「发表时间」picker 当前状态，供外部步骤（step_11）发表前校验使用。
...
def _schedule_matches_target(
    ...
    """校验 picker 状态是否与目标定时时间一致，供外部步骤（step_11）使用。"""
    if not state.get("ok"):
...
async def _close_picker(page: Page, wait_ms: int) -> bool:
    ...
    三步法（来自报告第二节验证）：
    1. 在 picker 矩形下方 60px 处真实点击（触发 v-click-outside）
    2. 点击「声明原创」标签（位于 picker 下方，点击不会触发额外弹窗）
    3. Escape 兜底
    """
    # 步骤 1：在 picker 矩形外侧（下方）点击
    xy_below = await _eval_xy(page, """
        const r = pickerRoot.getBoundingClientRect();
        if (r.width < 2 || r.height < 2) return null;
        const x = r.left + r.width * 0.35;
        const y = Math.min(r.bottom + 60, window.innerHeight - 10);
        if (y <= r.bottom + 6) return null;
        return { x, y };
    """)
    if await _real_click(page, xy_below):
        await page.wait_for_timeout(wait_ms)
        if not await _picker_is_open(page):
            logger.info("[视频号] picker 外侧点击已关闭弹窗")
            return True

    # 步骤 2：点击「声明原创」标签（在 picker 外部，不会触发其他弹窗）
    xy_original = await _eval_xy_shadow(page, """
        const labels = shadow.querySelectorAll('.form-item .label');
        for (const label of labels) {
        ...
        return null;
    """)
    if await _real_click(page, xy_original):
        await page.wait_for_timeout(wait_ms)
        if not await _picker_is_open(page):
            logger.info("[视频号] 点击「声明原创」已关闭弹窗")
            return True
    ...
    try:
        ...
        await page.wait_for_timeout(wait_ms)
        if not await _picker_is_open(page):
            logger.info("[视频号] Escape 键已关闭弹窗")
            return True
    except Exception as e:
        logger.debug("[视频号] Escape 兜底异常: %s", e)

    logger.warning("[视频号] 三步法均未能关闭弹窗")
    return False
...
class ScheduleSettingStep(BasePublishStep):
    ...
    async def execute(self, page: Page, file_path: str, metadata: Dict[str, Any]) -> StepOutcome:
        ...
        USER_LOG.info(
            "[步骤 8/11 定时发表] 时分已选择（%s:%s），准备关闭弹窗并校验",
            hour_str, minute_str,
        )
        ...
        if not closed:
            # 最后再检查一次：可能弹窗实际已关但检测有误
            if await _picker_is_open(page):
                return PublishResult(
                    success=False,
        ...
        USER_LOG.info(
            "[步骤 8/11 定时发表] ✓ 定时时间设置成功：%d-%02d-%02d %s:%s",
            t_year, t_month, t_day, hour_str, minute_str,
        )
        return None
