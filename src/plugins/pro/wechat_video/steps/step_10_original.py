...
"""
步骤10：声明原创
文件路径: src/plugins/pro/wechat_video/steps/step_10_original.py
...
async def wechat_main_original_checked(page: Page) -> Tuple[bool, str]:
    """只读：检测表单「声明原创」行是否已勾选（供步骤 11 发表前预检与步骤 10 幂等）。"""
    try:
...
""",
    ...
    async def execute(self, page: Page, file_path: str, metadata: Dict[str, Any]) -> StepOutcome:
        ...
        logger.info("[视频号] 步骤10：声明原创（is_original=%s）", is_original)
        USER_LOG.info("[步骤10/11 声明原创] ▶ 任务要求声明原创=%s", "是" if is_original else "否")

        if not is_original:
            logger.info("[视频号] 无需声明原创，跳过步骤10")
            USER_LOG.info("[步骤10/11 声明原创] — 跳过（任务未勾选）")
            return None
        ...
        if checked:
            ...
            USER_LOG.info("[步骤10/11 声明原创] ✓ 已是勾选状态，跳过")
            return None
        ...
        USER_LOG.info("[步骤10/11 声明原创] ▶ 已点击表单「声明原创」")
        ...
        if not has_dialog:
            ...
            if checked2:
                ...
                USER_LOG.info("[步骤10/11 声明原创] ✓ 无弹窗流程，主复选框已勾选")
                return None
        ...
        USER_LOG.info("[步骤10/11 声明原创] ▶ 已弹出「原创权益」窗口")
        ...
        for agree_attempt in range(1, 4):
            ...
            if not xy_ag:
                ...
                USER_LOG.info("[步骤10/11 声明原创] 协议勾选定位失败，重试 %s/3", agree_attempt)
                await page.wait_for_timeout(500)
                continue
            if not await real_mouse_click_xy(page, xy_ag):
                logger.warning("[视频号] 协议区域真实点击失败 (attempt=%s)，重试", agree_attempt)
                USER_LOG.info("[步骤10/11 声明原创] 协议点击失败，重试 %s/3", agree_attempt)
                await page.wait_for_timeout(500)
                continue
            ...
            if ok_ag:
                ...
                USER_LOG.info("[步骤10/11 声明原创] 协议已勾选（第 %s 次）", agree_attempt)
                break
        ...
        USER_LOG.info("[步骤10/11 声明原创] ▶ 已勾选「我已阅读并同意」")
        ...
        for attempt in range(1, 5):
            ...
            USER_LOG.info("[步骤10/11 声明原创] ▶ 已点弹窗「声明原创」")
            break
        ...
        USER_LOG.info("[步骤10/11 声明原创] ✓ 完成（主复选框已勾选）")
        logger.info("[视频号] 步骤10完成：声明原创成功")
        return None
