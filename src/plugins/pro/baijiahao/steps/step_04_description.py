...
"""
步骤4：作品描述（标题、简介、标签）
文件路径: src/plugins/pro/baijiahao/steps/step_04_description.py

流程：
  1. 标题填写：定位标题输入框（TITLE_INPUT），清空后逐字输入（百家号标题限 30 字）
  2. 简介填写：
     - 定位 contenteditable 编辑器或 textarea（DESC_EDITOR）
     - 拟人点击聚焦，全选清空旧内容
     - 逐字输入 description 全文
  3. 标签输入（可选）：
     - 定位标签输入框（TAG_INPUT）
