...
"""
步骤6：发布设置
文件路径: src/plugins/pro/baijiahao/steps/step_06_settings.py
...
字段依赖：
  - metadata['scheduled_publish_time'] / metadata['schedule_time']: 定时发布时间
  - metadata['privacy_settings']: 隐私设置 JSON（含 is_original）
    ...
    async def execute(self, page: Page, file_path: str, metadata: Dict[str, Any]) -> StepOutcome:
        ...
        if privacy_settings:
            try:
                ...
                is_original = privacy_settings.get("is_original", False)
            except Exception as e:
                logger.warning(f"解析 privacy_settings 异常: {e}")

        if is_original:
            logger.info("尝试勾选原创声明…")
            USER_LOG.info("[步骤6 发布设置] ▶ 尝试声明原创")
            for sel in Selectors.SETTINGS.get("ORIGINAL_CHECKBOX", []):
                try:
                    ...
                    if await loc.count() > 0 and await loc.is_visible():
                        ...
                        logger.info(f"已点击原创声明: {sel}")
                        USER_LOG.info("[步骤6 发布设置] ▶ 原创已声明")
                        break
                except Exception:
                    continue
