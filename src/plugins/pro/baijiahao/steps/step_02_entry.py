...
"""
步骤2：进入发布页
文件路径: src/plugins/pro/baijiahao/steps/step_02_entry.py

流程：
  1. 优先尝试直接导航到视频发布页 URL
  2. 若直接导航失败或未检测到发布页特征，回退到首页查找「发布」按钮点击
...
class EnterPublishEntryStep(BasePublishStep):
    ...
    async def execute(self, page: Page, file_path: str, metadata: Dict[str, Any]) -> StepOutcome:
        ...
        for selector in Selectors.HOME["VIDEO_PUBLISH_ENTRY"]:
            try:
                ...
                if await btn.count() > 0 and await btn.is_visible():
                    ...
                    logger.info(f"已点击发布视频入口: {selector}")
                    try:
                        from src.infrastructure.anti_risk.delays import random_delay
                        await random_delay(page, int(3000 * speed_rate), metadata, config)
                    except Exception:
    ...
    async def _check_publish_page_loaded(self, page: Page) -> bool:
        ...
        try:
            url = page.url
            if "edit" in url and "baijiahao.baidu.com" in url:
                return True
        except Exception:
            pass
