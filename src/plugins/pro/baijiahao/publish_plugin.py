...
"""
百家号发布插件（步骤链版本）
文件路径：src/plugins/pro/baijiahao/publish_plugin.py
...
def _platform_config() -> Dict[str, Any]:
    """从内置 config/platforms/baijiahao.json 读取平台配置。"""
    return load_platform_bundle("baijiahao")


def _load_limits() -> Dict[str, int]:
    """优先从 config/platforms/baijiahao.json 读取 limits，否则从插件 config.json，否则使用默认值。"""
    try:
...
class BaijiahaoPublishPlugin(PublishPluginInterface):
    """百家号发布插件 — 步骤链模式（7步）"""

    UPLOAD_URL = "https://baijiahao.baidu.com/builder/rc/edit?type=video"
    LOGIN_URL_KEYWORDS = ["passport.baidu.com", "login"]
    ...
    def get_form_schema(self, content_type: str = "video") -> List[FormField]:
        ...
        desc_max = limits.get("description_max_length", 5000)
        schema = [
            FormField(
                name="title",
                label="标题",
    ...
    async def publish(
        ...
        try:
            ...
            logger.error(f"百家号发布插件异常: {e}", exc_info=True)
