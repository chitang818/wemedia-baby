# -*- coding: utf-8 -*-
"""
步骤6：扩展信息 － 添加标签（位置/团购/购物车/小程序）尽力填写
文件路径: src/plugins/community/douyin/steps/step_06b_extra_info.py
...
class ExtraInfoCommonStep(BasePublishStep):
    ...
    async def _try_fill_add_tag_row(
            ...
            if goods and not (has_poi or anchor or mini):
                if await self._select_shopping_cart_via_openclaw_fallback(page, metadata, config):
                    ...
                    if link:
                        ...
                        if not cart_ok:
                            USER_LOG.error("[步骤6/8 扩展信息] ✗ 购物车商品未成功添加，终止发布")
                            return PublishResult(success=False, error_message="购物车商品添加失败，「已添加商品」区域未出现", failed_step="步骤6/扩展信息")
                    elif promo:
                        await self._fill_promotion_if_present(page, promo, metadata, config)
                    USER_LOG.info("[步骤6/8 扩展信息] ✓ 已填写「购物车」标签，商品已添加")
                    return None
                else:
                    USER_LOG.error("[步骤6/8 扩展信息] ✗ 未找到「添加标签」及购物车下拉，终止发布")
                    return PublishResult(success=False, error_message="未找到「添加标签」及购物车下拉", failed_step="步骤6/扩展信息")
            USER_LOG.error("[步骤6/8 扩展信息] ✗ 未找到「添加标签」行，终止发布（已配置%s标签）", tag_type)
            return PublishResult(
                success=False,
                error_message=f"未找到「添加标签」行，无法填写{tag_type}标签",
                failed_step="步骤6/扩展信息",
            )
        ...
        try:
            if has_poi:
                ...
                USER_LOG.info("[步骤6/8 扩展信息] ✓ 已填写「位置」标签")
                return None
            if anchor:
                ...
                if promo:
                    await self._fill_promotion_if_present(page, promo, metadata, config)
                USER_LOG.info("[步骤6/8 扩展信息] ✓ 已填写「团购」标签")
                return None
            if goods:
                link, promo = _parse_goods_storage(goods)
            ...
            if mini:
                ...
                USER_LOG.info("[步骤6/8 扩展信息] ✓ 已填写「小程序」标签")
                return None
        except Exception as e:
            logger.error(f"扩展信息：填写「{tag_type}」标签失败: {e}", exc_info=True)
            USER_LOG.error("[步骤6/8 扩展信息] ✗ 「%s」标签填写失败，终止发布: %s", tag_type, str(e)[:100])
            return PublishResult(
                success=False,
    ...
    async def _fill_cart_link_and_confirm(
        ...
        try:
            ...
            if cart_input is None:
                ...
                USER_LOG.warning("[步骤6/8 扩展信息] ✗ 购物车：未找到链接输入框")
                return False
            ...
            if not await self._click_cart_add_link_span(page, metadata, config):
                ...
                USER_LOG.warning("[步骤6/8 扩展信息] ✗ 购物车「编辑商品」弹窗未出现（链接可能无效）")
                return False
            ...
            USER_LOG.warning("[步骤6/8 扩展信息] ✗ 购物车设置未完成（可手动检查）: %s", str(e)[:80])
            return False
    ...
    async def _handle_edit_goods_modal(
        ...
        if await finish_btn.count() == 0:
            ...
            USER_LOG.warning("[步骤6/8 扩展信息] ✗ 购物车：未找到「完成编辑」按钮")
            return False
        ...
        if await self._verify_cart_goods_added(page):
            ...
            USER_LOG.warning("[步骤6/8 扩展信息] ✗ 购物车：未检测到「已添加商品」，商品可能未成功添加")
            return False
